...
- **[`learnings/AGENTS.md`](learnings/AGENTS.md)** — Read this **before adding or editing a lesson**. Covers the **base-path link trap** (`/fundamentals-bootcamp/`), which link style each file location needs, the auto-generated lesson sidebar, self-contained exercise HTML, and the pre-finish link audit. Written docs under `reference-site/topics/` follow *this* file instead.
